...
const App: React.FC = () => {
  const [user, setUser] = useState<User | null>(() => safeLoad('hospflow_session_v5'));
  const [selectedUnit, setSelectedUnit] = useState<string | null>(null);
  const [currentView, setCurrentView] = useState<AppView>(() => {
    return safeLoad('hospflow_session_v5') ? 'UNIT_SELECTION' : 'LOGIN';
  });
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (found.password === password) {
      ...
      if (rememberMe) {
        safeSave('hospflow_session_v5', authUser);
      } else {
      ...
      if (blocked) {
        alert("USUÁRIO BLOQUEADO: Você errou a senha 3 vezes. Procure a Coordenação.");
      } else {
        alert(`SENHA INCORRETA: Verifique sua senha numérica. Tentativa ${attempts} de 3. Após 3 erros o usuário será bloqueado.`);
      }
    }
  };
  ...
  const updatePatients = useCallback((ids: string[], updates: Partial<Patient>) => {
    setPatients(prev => prev.map(p => {
      if (ids.includes(p.id)) {
        return { ...p, ...updates, lastModifiedBy: `${user?.username} - ${user?.name}` };
      }
      return p;
    }).sort((a, b) => a.name.localeCompare(b.name)));
  }, [user]);
  ...
  const renderContent = () => {
    switch (currentView) {
      case 'MAIN_MENU':
        ...
        // Mapeamento dos botões para facilitar a ordenação
        const menuButtons = {
          decisaoClinica: (
          ...
          transferirPacientes: (
            <button key="transferir" onClick={() => setCurrentView('TRANSFERS')} className={btnBase}>
        ...
        // Ordem específica para o perfil de Coordenação
        ...
        // Ordem específica para o perfil de Enfermeiro
        const nurseMenu = [
          menuButtons.decisaoClinica,
          menuButtons.listaPacientes,
          menuButtons.novoPaciente,
        ...
        // Ordem específica para o perfil de Técnico
              ...
              <form onSubmit={handleLogin} className="p-8 space-y-6">
                <div className="flex bg-slate-100 p-1 rounded-2xl gap-1">
                  {['tecnico', 'enfermeiro', 'coordenacao'].map((r) => (
                    <button 
                      ...
                      className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${selectedRole === r ? 'bg-white shadow-md text-blue-600' : 'text-slate-400 hover:bg-slate-200/50'}`}
                    >
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <input required type="text" inputMode="numeric" placeholder="USUÁRIO" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-bold" value={username} onChange={e => setUsername(e.target.value.replace(/\D/g, ''))} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Senha (Números)</label>
                    <input required type="password" inputMode="numeric" placeholder="SENHA" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none font-bold" value={password} onChange={e => setPassword(e.target.value.replace(/\D/g, ''))} />
                  </div>
                </div>
                <div className="flex items-center gap-2 mt-4 cursor-pointer select-none">
                  <input id="remember" type="checkbox" checked={rememberMe} onChange={e => setRememberMe(e.target.checked)} className="w-4 h-4 accent-blue-600 rounded" />
                  <label htmlFor="remember" className="text-[10px] font-black text-slate-500 uppercase tracking-widest cursor-pointer">Manter conectado</label>
                </div>
